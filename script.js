...
formTable.addEventListener("submit", (event)=>{
  ...
  // if(parseInt(tableMap.get(`a${i}z`)) + parseInt(tableMap.get(`b${i}p`))
  ...
  for(let i=1;i<=4;i++){
    for(let j=1;j<=4;j++){
      ...
      if((i % 4 === 0)&& (j % 4 === 0)) {
        ...
        pSum += parseInt(document.getElementById(`b${j}p`).value);

      }
      else if(j % 4 === 0){
        tableMap.set(`a${i}z`, document.getElementById(`a${i}z`).value);
        zSum += parseInt(document.getElementById(`a${i}z`).value);
      }
      else{
  ...
  if(zSum !== pSum) {
    ...
    document.querySelector('.page__text--1').textContent = firstResult.toString();
  }
